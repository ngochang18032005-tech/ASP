...
## CHƯƠNG XVIII: HƯỚNG DẪN DEPLOY CLOUD & XỬ LÝ SỰ CỐ (CLOUD DEPLOYMENT)

Phần này ghi lại các kinh nghiệm thực tế khi đưa hệ thống LuxManage lên các môi trường như Render, Azure hoặc AWS.

### 18.1. Cấu hình biến môi trường (Environment Variables)
Khi chạy trên Cloud, hệ thống sẽ ưu tiên đọc cấu hình từ biến môi trường thay vì file `appsettings.json`.
*   **Biến quan trọng nhất:** `ConnectionStrings__DefaultConnection`.
*   **Tác dụng:** Chứa chuỗi kết nối tới Database online.
*   **Lưu ý:** Không bao giờ để tên máy tính cá nhân (ví dụ `HANG\SQLEXPRESS`) trong biến này khi chạy trên Cloud.

### 18.2. Xử lý lỗi phổ biến
1.  **Lỗi Status 139 (Segmentation Fault):** Thường xảy ra khi ứng dụng sập ngay lúc khởi động do không kết nối được Database. Hệ thống LuxManage hiện đã có cơ chế "Startup Resilience" (Bảo vệ khởi động) giúp ứng dụng vẫn chạy được để báo lỗi thay vì sập hoàn toàn.
2.  **Cảnh báo Decimal Precision:** Khi làm việc với các hệ quản trị database khác nhau, độ chính xác của cột tiền tệ có thể bị sai lệch. Hệ thống đã cấu hình cứng chuẩn `(18, 2)` trong `OnModelCreating` để đảm bảo tính minh bạch tài chính.

### 18.3. Quy trình Deploy lên Render (Tóm tắt)
1.  Kết nối GitHub Repository với Render.
2.  Chọn môi trường là **Docker**.
3.  Thêm biến môi trường cho Database.
4.  Bấm **Deploy** và tận hưởng thành quả.

---
**LuxManage Resort - Hệ thống quản lý nghỉ dưỡng hàng đầu cho kỷ nguyên số.**
*(Tài liệu chi tiết kỹ thuật chuyên sâu - Siêu tài liệu v1.0.1)*
